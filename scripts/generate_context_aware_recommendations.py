...
def get_project_context() -> Dict:
    ...
    context_questions = {
        "project_name": "Tên project/dự án? (ví dụ: HN-DC03)",
...
def generate_hybrid_architecture(context: Dict) -> str:
    ...
    if "quick" in timeline.lower() or "demo" in timeline.lower():
        architecture.append("✅ **Layer 1 - Production PoC:** Dify + FastGPT")
...
def generate_named_use_cases(context: Dict) -> List[Dict]:
    ...
    if "vendor" in business_need.lower() or "review" in business_need.lower():
        use_cases.append({
            "name": f"review_{project_name.lower().replace(' ', '_')}_vendor_matrix",
    ...
    if "document" in business_need.lower() or "qa" in business_need.lower():
        use_cases.append({
            "name": f"{project_name.lower().replace(' ', '_')}_document_qa_bot",
            "description": "Q&A over project documents (HSYC, HSMT, contracts, specs)",
            "tool": "FastGPT",
    ...
    if "report" in business_need.lower() or "leader" in business_need.lower():
        use_cases.append({
            "name": f"generate_{project_name.lower().replace(' ', '_')}_executive_report",
    ...
    if not use_cases:
        use_cases.append({
            "name": f"{project_name.lower().replace(' ', '_')}_ai_assistant",
...
def generate_recommendation_report(context: Optional[Dict] = None) -> str:
    ...
    report = []
    report.append("# 🎯 Khuyến Nghị Sử Dụng Công Cụ AI Trung Quốc")
...
if __name__ == "__main__":
    print("🚀 Context-Aware AI Tool Recommendation Generator")
    print("=" * 60)
